...
wandb.init(project="name", name="llama3.1_sft", config={"model": MODEL_ID, "learning_rate": 2e-5})
...
def main():
    ...
    max_length = 8192
    train_dataset = Dataset.from_parquet("./dataset/sft/train_dataset_1205_a_llama.parquet")
    eval_dataset = Dataset.from_parquet("./dataset/sft/train_dataset_1205_a_llama.parquet")
    ...
    trainer = SFTTrainer(
        ...
        args=training_args,
        # callbacks=[EvalCallback()],  # Callback 추가
    )
    ...
    trainer.save_model("./output_llama/final_model")
    tokenizer.save_pretrained("./output_llama/final_model")
    print("Model saved to ./output_llama/final_model")
