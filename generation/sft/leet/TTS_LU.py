...
PROMPT_MAP = {
    "<보기>기반 추론": PROMPT_OPTION_REASONING,
    "<보기>기반 추론- ㄱ,ㄴ,ㄷ": PROMPT_OPTION_1_REASONING,
    "지문 기반 추론":  PROMPT_LOGIC_REASONING,
    "지문 기반 추론 (글쓴이의 견해)": PROMPT_AUTHOR_REASONING,
    "주장/요소 비교 (밑줄)": PROMPT_LINE_REASONING,
    "주장/요소 비교 (ㄱ, ㄴ)": PROMPT_AB_REASONING,

    "사실 확인 문제": PROMPT_OX_REASONING,
    "요소 이해 문제": PROMPT_OX_REASONING,
    "내용 이해 문제": PROMPT_OX_REASONING,
}

def lu_social_inference(input_question, prompt_template):
    """단일 질문에 대한 o3 모델 추론"""
    ...
    response = client.responses.create(
        ...
        input=[
            {
                "role": "developer",
                "content": "You are assisting with tasks from the Language Understanding (언어이해) section of the Korean LEET (Law-school Entrance Exam)."
            },
            {
                "role": "user",
                "content": prompt
...
def process_single_question(row_data):
    ...
    try:
        result, input_tokens, output_tokens, reasoning_trace, reasoning_tokens = lu_social_inference(question, prompt_template)
...
def main():
    ...
    df = pd.read_csv("./dataset/LEET/언어이해_final.csv")
    df_filtered = df[df['유형'].isin(["사실 확인 문제", "내용 이해 문제", "요소 이해 문제"])]
    ...
    if use_parallel:
        ...
        for i, row in tqdm(df_filtered.iterrows(), total=len(df_filtered), desc="Processing questions (sequential)"):
            ...
            df_output.loc[i, 'output_token.s'] = output_tokens
